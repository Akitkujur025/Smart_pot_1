    ...
    const fetchRealtimeData = async () => {
      try {
        ...
        dataArray.forEach(function (item, index) {
          ...
          if(item.Temp.C>95){
            $(`#plantMessage`).text(`Too Hot - Place the pot in coolder environment \nTemperature: ${temperature}`);
          }
          else if(item.Humidity<10){
            $(`#plantMessage`).text(`Too Dry Humidity - Place the pot in humid environment \nHumidity: ${humidity}`);
          }
          else if(item.Humidity>95){
            $(`#plantMessage`).text(`Too Humid Humidity - Place the pot in dry environment \nHumidity: ${humidity}`);
          }
          else{
            $(`#plantMessage`).text(`Plant in Good Condition`);
          }
